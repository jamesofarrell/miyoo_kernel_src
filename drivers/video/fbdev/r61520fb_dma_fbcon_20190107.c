...
#define IOCTL_SET_GAME_MODE   _IOW (IOCTL_BASE, 2, unsigned long)
#define IOCTL_FLUSH_DMA       _IOW (IOCTL_BASE, 3, unsigned long)
...
static void sunxi_lcdc_gpio_config(uint32_t use_gpio)
{
  ...
  ret = readl(iomm.gpio + PE_CFG1); // tear pin
  ret&= 0xfffff0ff;
  writel(ret, iomm.gpio + PE_CFG1);
  ret = readl(iomm.gpio + PE_DRV0);
  ret&= 0xffcfffff;
  ret|= 0x00300000;
  writel(ret, iomm.gpio + PE_DRV0);
...
static void sunxi_dma_init(struct myfb_par *par)
{
  ...
  writel(10000, iomm.dma + DDMA1_BYTE_CNT_REG);
  // dma 2
...
static int myfb_ioctl(struct fb_info *info, unsigned int cmd, unsigned long arg)
{
  ...
  case IOCTL_GET_DMA_PHYS:
    copy_to_user((void*)arg,  &par->lram_phys[0], sizeof(unsigned long));
    printk("IOCTL_GET_DMA_PHYS 0x%x\n", &par->lram_phys[0]);
    break;
  case IOCTL_SET_GAME_MODE:
    par->game_mode = arg;
    printk("IOCTL_SET_GAME_MODE %d\n", par->game_mode);
    break;
  case IOCTL_FLUSH_DMA:
    //ret = readl(iomm.gpio + PE_DATA) & (1 << 10);
    //if(ret){
      //sunxi_dma_start(par);
    //}
    break;
...
static int myfb_pan_display(struct fb_var_screeninfo *var, struct fb_info *info)
{
  ...
  if((var->xoffset != info->var.xoffset) || (var->yoffset != info->var.yoffset)){
    ...
    memcpy(par->tram_virt, addr, size);
    sunxi_dma_fill(par);
    sunxi_dma_start(par);
  }
  return 0;
...
{
  uint32_t ret, ch1=1;
  static uint32_t pre_tear=0;
  struct myfb_par *par = (struct myfb_par*)arg;

  //printk("%s", __func__);
  writel(0xffffffff, iomm.dma + DMA_INT_STA_REG);
  ret = (readl(iomm.gpio + PE_DATA) & (1 << 10)) ? 1 : 0;
  if(pre_tear != ret){
    pre_tear = ret;
    if(ret){
      ch1 = 0;
