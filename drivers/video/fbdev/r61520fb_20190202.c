...
static void suniv_lcdc_init(struct myfb_par *par)
{
  uint32_t ret, bp, total;
	uint32_t pixel_clock_hz = 8300000;
...
#else
  ret = gpio_request(SLCD_TE, "slcd_te");
  gpio_direction_input(SLCD_TE);
  gpio_set_debounce(SLCD_TE, 10);
  par->irq = gpio_to_irq(SLCD_TE);
  if (par->irq < 0) {
    printk("%s, failed to get irq number\n", __func__);
  }
  else{
	  ret = request_irq(par->irq, lcdc_irq_handler, IRQF_TRIGGER_RISING, "miyoo_lcdc_irq", par);
	  if(ret){
		  printk("%s, failed to register LCDC interrupt(%d)\n", __func__, par->irq);
	  }
  }
