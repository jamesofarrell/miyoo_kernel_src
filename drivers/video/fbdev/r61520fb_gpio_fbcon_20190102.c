...
static void sunxi_dma_init(const struct r61520fb_par *par)
{
  ...
  writel(0x00000002, iomm.dma + DMA_INT_STA_REG);
  writel(0x00010000, iomm.dma + DMA_PTY_CFG_REG);
  writel(0x21280108, iomm.dma + NDMA0_CFG_REG);
  ...
  //writel(iomm.gpio + PD_DATA, iomm.dma + NDMA0_DES_ADR_REG); 
  //writel(1, iomm.dma + NDMA0_BYTE_CNT_REG);
  //setbits(iomm.dma + NDMA0_CFG_REG, 0x80000000);
...
static long efb_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
  ...
  case IOCTL_TURN_OFF_WORK:
    work_stop = 1;
    /*writel(fbpar->tram_phys, iomm.dma + NDMA0_SRC_ADR_REG);
    //writel(par->tram_virt, iomm.dma + NDMA0_SRC_ADR_REG);
    writel(SUNXI_GPIO_BASE + PD_DATA, iomm.dma + NDMA0_DES_ADR_REG);
    //writel(iomm.gpio + PD_DATA, iomm.dma + NDMA0_DES_ADR_REG); 

    uint32_t *p = fbpar->tram_virt;
    p[0] = 3;
