...
static void suniv_gpio_init(void)
{
  ...
  /*
  ret = readl(iomm.gpio + PE_INT_CFG1);
  ret&= 0xfffff0ff;
  writel(ret, iomm.gpio + PE_INT_CFG1);
  suniv_setbits(iomm.gpio + PE_INT_CTRL, (1 << 10));
...
static irqreturn_t lcdc_irq_handler(int irq, void *arg)
{
  ...
  /*if(par->lram_ready){
    par->lram_ready = 0;
    memcpy(par->lram_virt[1], par->lram_virt[0], 320*240*2);
  }*/
  return IRQ_HANDLED;
}
...
static void suniv_lcdc_init(struct myfb_par *par)
{
  uint32_t ret, bp, total;
	uint32_t pixel_clock_hz = 8000000;
  ...
  // enable interrupt
  par->irq = platform_get_irq(par->pdev, 0);
  if (par->irq < 0) {
    printk("%s, failed to get irq number\n", __func__);
  }
  else{
	  ret = request_irq(par->irq, lcdc_irq_handler, IRQF_SHARED, "miyoo_lcdc_irq", par);
	  if(ret){
...
static int myfb_pan_display(struct fb_var_screeninfo *var, struct fb_info *info)
{
  ...
  //if((var->xoffset != info->var.xoffset) || (var->yoffset != info->var.yoffset)){
    ...
    size = (new_var.xres * new_var.yres * info->var.bits_per_pixel) / 8;
    offset = (new_var.yoffset * fix->line_length) + ((new_var.xoffset * info->var.bits_per_pixel) / 8);
    //memcpy(par->lram_virt[0], par->vram_virt + offset, size);
    //par->lram_ready = 1;
  //}
  return 0;
}
...
static int myfb_probe(struct platform_device *device)
{
  ...
  //printk("%s, xres: %d, yres:%d, bpp:%d\n", __func__, mode->xres, mode->yres, par->bpp);
