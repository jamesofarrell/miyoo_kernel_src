...
struct r61520_fb_par {
  ...
  void *lram_virt[8];
  unsigned long lram_index;
  unsigned long lram_section;
...
static void sunxi_dma_start(struct r61520_fb_par *par, uint8_t wait)
{
  writel(1024 * 128, iomm.dma + NDMA0_BYTE_CNT_REG);
  writel(2, iomm.dma + DMA_INT_STA_REG);
  setbits(iomm.dma + NDMA0_CFG_REG, (1 << 31));
  if(wait){
    while((readl(iomm.dma + DMA_INT_STA_REG) & 2) == 0);
  }
  par->lram_index = 0;
}

static void sunxi_dma_push(struct r61520_fb_par *par, uint32_t is_data, uint32_t val)
{
  uint32_t ret;
  uint32_t *src = par->lram_virt[0];

  ret = (val & 0x00ff) << 1;
  ret|= (val & 0xff00) << 2;
  ret|= is_data ? 0x80000 : 0;
  ret|= 0x100000;
  src[par->lram_index++] = ret;
  ret|= 0x40000;
  src[par->lram_index++] = ret;
}
...
static void sunxi_lcdc_init(struct r61520_fb_par *par)
{
  ...
  writel((1 << 31), iomm.lcdc + TCON_CTRL_REG);
    ...
    writel(0x2c, iomm.lcdc + TCON0_CPU_WR_REG);

    // data
    while(readl(iomm.lcdc + TCON0_CPU_IF_REG) & (1 << 23));
    setbits(iomm.lcdc + TCON0_CPU_IF_REG, 0x2000000);
    writel(*p++, iomm.lcdc + TCON0_CPU_WR_REG);
  #endif
}
