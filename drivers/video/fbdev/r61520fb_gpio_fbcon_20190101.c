...
static void sunxi_dma_init(const struct r61520fb_par *par)
{
  ...
  writel(0x00020000, iomm.dma + DMA_INT_STA_REG);
  writel((1 << 16), iomm.dma + DMA_PTY_CFG_REG);
  writel((1 << 29) | (1 << 28) | (1 << 25) | (1 << 21) | (1 << 17) | (1 << 9) | (1 << 0), iomm.dma + DDMA0_CFG_REG);
  writel(par->tram_phys, iomm.dma + DDMA0_SRC_ADR_REG);
  //writel(par->tram_virt, iomm.dma + DDMA0_SRC_ADR_REG);
  writel(SUNXI_GPIO_BASE + PD_DATA, iomm.dma + DDMA0_DES_ADR_REG);
  //writel(iomm.gpio + PD_DATA, iomm.dma + DDMA0_DES_ADR_REG); 
  writel(320*240*4, iomm.dma + DDMA0_BYTE_CNT_REG);
  //setbits(iomm.dma + DDMA0_CFG_REG, 0x80000000);
...
static void lcd_refresh_work(struct work_struct *work)
{
  ...
  if(par->is_enabled){
    ...
    schedule_delayed_work(&par->refresh_work, HZ / 15);
  }
