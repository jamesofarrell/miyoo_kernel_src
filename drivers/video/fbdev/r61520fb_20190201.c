...
static irqreturn_t lcdc_irq_handler(int irq, void *arg)
{
  ...
  if(myosd_val & (1 << 31)){
    if(myosd_frame){
      ...
      for(i=0; i<10; i++){
        draw_osd(par->lram_virt[0], 40 + (i*24), 20, 0xf800, ((i <= lid) ? 0xf800 : 0));
        draw_osd(par->lram_virt[0], 40 + (i*24), 200, 0x001f, ((i <= vol) ? 0x001f : 0));
      }
      myosd_frame-= 1;
    }
  }
