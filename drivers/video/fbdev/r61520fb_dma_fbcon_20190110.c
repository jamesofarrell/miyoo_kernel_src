...
static void sunxi_lcdc_gpio_config(uint32_t use_gpio)
{
  ...
  setbits(iomm.gpio + PE_INT_CTRL, (1 << 10)); // enable interrupt
...
static uint32_t lcdc_wait_busy(void)
{
	...
	val = readl(iomm.lcdc + TCON0_CPU_IF_REG);
	val|= (1 << 0);
	writel(val, iomm.lcdc + TCON0_CPU_IF_REG);
	ndelay(10);
	while(1){
		val = readl(iomm.lcdc + TCON0_CPU_IF_REG);
		if(val & 0x00c00000){
...
static void lcdc_cmd(uint32_t cmd)
{
	...
	val = readl(iomm.lcdc + TCON0_CPU_IF_REG);
	val&= ~(1<<25);
	writel(val, iomm.lcdc + TCON0_CPU_IF_REG); // ca=0
	while(lcdc_wait_busy());
...
static void lcdc_dat(uint32_t data)
{
	...
	val = readl(iomm.lcdc + TCON0_CPU_IF_REG);
	val|= (1 << 25);
	writel(val, iomm.lcdc + TCON0_CPU_IF_REG); // ca=1
	while(lcdc_wait_busy());
...
{
  printk("%s\n", __func__);
  clrbits(iomm.lcdc + TCON_INT_REG0, (1 << 15));
  return IRQ_HANDLED;
}
...
static void sunxi_lcdc_init(struct myfb_par *par)
{
  uint32_t ret, i, bp, total;
	uint32_t pixel_clock_hz = 33000000;
  ...
  setbits(iomm.ccm + PLL_VIDEO_CTRL_REG, (1 << 4));
  setbits(iomm.ccm + PLL_PERIPH_CTRL_REG, (1 << 4));
  setbits(iomm.ccm + PLL_VE_CTRL_REG, (1 << 4));
  setbits(iomm.ccm + FE_CLK_REG, (1 << 31) | (1 << 0));
  setbits(iomm.ccm + BE_CLK_REG, (1 << 31) | (1 << 0));
  setbits(iomm.ccm + DI_CLK_REG, (1 << 31) | (1 << 0));
  setbits(iomm.ccm + TCON_CLK_REG, (1 << 31));
  setbits(iomm.ccm + BUS_CLK_GATING_REG1, (1 << 14));
  setbits(iomm.ccm + BUS_CLK_GATING_REG1, (1 << 12));
  setbits(iomm.ccm + BUS_CLK_GATING_REG1, (1 << 5));
  setbits(iomm.ccm + BUS_CLK_GATING_REG1, (1 << 4));
  ...
  // f1c100s_tcon_cpu_set_mode
	...
	// f1c100s_tcon_enable
	setbits(iomm.lcdc + TCON_CTRL_REG, (1 << 31));

	// fb_f1c100s_lcd_init
	setbits(iomm.lcdc + TCON_INT_REG0, (1 << 31));
	setbits(iomm.lcdc + TCON0_CPU_IF_REG, (1 << 28));
}
...
static int myfb_pan_display(struct fb_var_screeninfo *var, struct fb_info *info)
{
  ...
  if(par->refill_mode == MIFB_REFILL_FULL_AUTO){
    return 0;
  }
...
static int myfb_probe(struct platform_device *device)
{
  ...
  /*memcpy(par->tram_virt, par->vram_virt, 320*240*2);
  {
    uint32_t cnt=0;
    lcdc_cmd(0x2c);
