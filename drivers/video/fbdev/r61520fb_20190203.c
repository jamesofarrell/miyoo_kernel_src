...
{
/*
  writel((uint32_t)(par->vram_phys + par->cur_line) << 3, iomm.debe + DEBE_LAY0_FB_ADDR_REG);
	writel((uint32_t)(par->vram_phys + par->cur_line) >> 29, iomm.debe + DEBE_LAY0_FB_HI_ADDR_REG);
  writel((uint32_t)(par->vram_phys + 320*240*2 + par->cur_line) << 3, iomm.debe + DEBE_LAY1_FB_ADDR_REG);
	writel((uint32_t)(par->vram_phys + 320*240*2 + par->cur_line) >> 29, iomm.debe + DEBE_LAY1_FB_HI_ADDR_REG);
  par->cur_line+= SCANLINE;
  if(par->cur_line >= 240){
    par->cur_line = 0;
    //lcdc_cmd(0x2c);
  }
...
static void r61520_init(void)
{
  ...
  lcdc_dat(0x02); // division 0x01
  lcdc_dat(0x11); // clock 0x11
  lcdc_dat(PORCH); // bp0 0x08
  lcdc_dat(PORCH); // fp0 0x08

  lcdc_cmd(0xc3);
  lcdc_dat(0x01); // line inversion
  lcdc_dat(0x02); // division 0x01
  lcdc_dat(0x11); // clock 0x11
  lcdc_dat(PORCH); // bp2 0x08
  lcdc_dat(PORCH); // fp2 0x08
...
static void suniv_lcdc_init(struct myfb_par *par)
{
  uint32_t ret, bp, total;
	uint32_t pixel_clock_hz = 2000000;
	uint32_t h_front_porch = PORCH; // 40
	uint32_t h_back_porch = PORCH; // 87
	uint32_t h_sync_len = 1;
	uint32_t v_front_porch = PORCH; // 13
	uint32_t v_back_porch = PORCH; // 31
	uint32_t v_sync_len = 1;
  uint32_t yres = par->mode.yres;
  uint32_t xres = par->mode.xres;
...
static void suniv_cpu_init(struct myfb_par *par)
{
  ...
  writel(0x1ff, iomm.ccm + PLL_STABLE_TIME_REG0);
  writel(0x1ff, iomm.ccm + PLL_STABLE_TIME_REG1);

  ret = readl(iomm.ccm + CPU_CLK_SRC_REG);
  ret&= ~(3 << 16);
  ret|= (1 << 16);
  writel(ret, iomm.ccm + CPU_CLK_SRC_REG);

  writel(0x81004107, iomm.ccm + PLL_VIDEO_CTRL_REG);
  while(!(readl(iomm.ccm + PLL_VIDEO_CTRL_REG) & (1 << 28)));
  writel(0x80041800, iomm.ccm + PLL_PERIPH_CTRL_REG);
  while(!(readl(iomm.ccm + PLL_PERIPH_CTRL_REG) & (1 << 28)));
  writel(0x00003180, iomm.ccm + AHB_APB_HCLKC_CFG_REG);
  ...
  ret = readl(iomm.ccm + PLL_CPU_CTRL_REG);
	writel(ret, iomm.ccm + PLL_CPU_CTRL_REG);
	ret = readl(iomm.ccm + CPU_CLK_SRC_REG);
	ret&= ~(3 << 16);
	ret|= (2 << 16);
	writel(ret, iomm.ccm + CPU_CLK_SRC_REG);
