...
#define MIFB_REFILL_MANUAL_SET  2
...
static void sunxi_lcdc_gpio_config(uint32_t use_gpio)
{
  ...
  ret = readl(iomm.gpio + PE_INT_CTRL); // enable interrupt
  ret|= (1 << 10);
  writel(ret, iomm.gpio + PE_INT_CTRL);
...
static int myfb_ioctl(struct fb_info *info, unsigned int cmd, unsigned long arg)
{
  ...
  case OMAPFB_SETUP_MEM:
    ...
    break;
  case MIFB_GET_DMA:
    copy_from_user(&index, arg, sizeof(unsigned long));
    if(index >= DMA_NUM){
