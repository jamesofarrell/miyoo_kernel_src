...
static void r61520_lcd_cmd(uint32_t val)
{
  ...
  if (iomm.use_gpio){
    ...
    if (iomm.expand_16bit) {
      writel(((val & 0xfc00) << 8) | ((val & 0x300) << 6) | ((val & 0xe0) * 32) | ((val & 0x1f) * 8), &iomm.lcdc->tcon0_cpu_wr_dat);
    }
    else {
...
static void r61520_lcd_dat(uint32_t val)
{
  ...
  if (iomm.use_gpio) {
    ...
    if (iomm.expand_16bit) {
      writel(((uint32_t)(val & 0xfc00) << 8) | ((val & 0x300) << 6) | ((val & 0xe0) * 32) | ((val & 0x1f) * 8), &iomm.lcdc->tcon0_cpu_wr_dat);
    }
    else {
      writel(val, &iomm.lcdc->tcon0_cpu_wr_dat);
    }
...
static void sunxi_lcdc_gpio_config(uint32_t use_gpio, uint32_t expand_16bit)
{
  ...
  if (iomm.use_gpio) {
    writel(0x11111117, &iomm.gpio->gpio_bank[3].cfg[0]); // 0x11111117
    writel(0x11111171, &iomm.gpio->gpio_bank[3].cfg[1]); // 0x11111171
    writel(0x00111111, &iomm.gpio->gpio_bank[3].cfg[2]); // 0x00111111, CS/RD/RS/WR
  }
  else {
    writel(0x22222227, &iomm.gpio->gpio_bank[3].cfg[0]); // 0x22222221
    writel(0x22222272, &iomm.gpio->gpio_bank[3].cfg[1]); // 0x22222212
...
static void sunxi_lcdc_init(const struct r61520fb_par *par)
{
  ...
  printk("%s, video pll(before): %dMHz\n", __func__, ret/1000000);
  clock_set_video_pll(12000000);
  ret = clock_get_video_pll();
  printk("%s, video pll(after): %dMHz\n", __func__, ret/1000000);
  
  ret = readl(&iomm.ccm->cpu_clk_src);
  printk("%s, cpu_clk_src: 0x%08x\n", __func__, ret);
  ...
  writel(0x00000000, &iomm.lcdc->ctrl);
	...
	writel(0x01400003, &iomm.lcdc->tcon0_ctrl);
  writel(0x00000000, &iomm.lcdc->tcon0_dclk);
  writel(0x013f00ef, &iomm.lcdc->tcon0_timing_active);
  writel(0x81e00003, &iomm.lcdc->tcon0_timing_h);
  writel(0x03840003, &iomm.lcdc->tcon0_timing_v);
  writel(0x00000000, &iomm.lcdc->tcon0_timing_sync);
  writel(0x00000000, &iomm.lcdc->tcon0_hv_intf);
  writel(0x84000000, &iomm.lcdc->tcon0_cpu_intf);
  ...
  writel(0x00000000, &iomm.lcdc->tcon0_io_tristate);
	...
	writel(0x00000000, &iomm.lcdc->tcon0_frm_ctrl);
  writel(0x80000000, &iomm.lcdc->ctrl);
	writel(0x814000f0, &iomm.lcdc->tcon0_ctrl);
  writel(0x80000000, &iomm.lcdc->tcon0_dclk);
}
