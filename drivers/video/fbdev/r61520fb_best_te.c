...
static void r61520_init(void)
{
  ...
  lcdc_dat(0x00); // division 0x01
  lcdc_dat(0x11); // clock 0x11
  lcdc_dat(0x08); // bp0 0x08
  lcdc_dat(0x08); // fp0 0x08

  lcdc_cmd(0xc3);
  lcdc_dat(0x01); // line inversion
  lcdc_dat(0x00); // division 0x01
  lcdc_dat(0x11); // clock 0x11
  lcdc_dat(0x08); // bp2 0x08
  lcdc_dat(0x08); // fp2 0x08
...
static void suniv_lcdc_init(struct myfb_par *par)
{
  ...
  // set tcon cpu mode
	...
	ret = ((297000000 / pixel_clock_hz) / 2);
  ...
  gpio_set_debounce(SLCD_TE, 1000/65);
