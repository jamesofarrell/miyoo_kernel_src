...
static void suniv_codec_init(void)
{
  ...
  writel((1 << 31) | (1 << 30) | (1 << 27) | (1 << 26) | (1 << 17) | (1 << 15) | (1 << 9) | (0x3b << 0), iomm.codec + DAC_MIXER_CTRL_REG);
  ...
  #if 1
  {
    ...
    for(i=0; i<1024; i++){
      for(j=0; j<8192; j++){
        writel(s, iomm.codec + AC_DAC_TXDATA_REG);
      }
      s = s ? 0 : 0x7f;
    }
  }
...
static irqreturn_t dma_irq_handler(int irq, void *arg)
{
  ...
  if(ret & 2){
    if(iomm.dma_trigger){
      ...
      //printk("%s, start 0x%08x, end 0x%08x, pos 0x%08x, period %d\n", __func__, prtd->dma_start, prtd->dma_end, prtd->dma_pos, prtd->dma_period);
    }
  }
  return IRQ_HANDLED;
}
...
static int mypcm_hw_params(struct snd_pcm_substream *substream, struct snd_pcm_hw_params *params)
{
  ...
  //printk("%s\n", __func__);
	if(prtd == NULL) {
		printk("%s, mypcm is NULL\n", __func__);
		return 0;
	}

  switch (params_channels(params)) {
  case 2:
    suniv_clrbits(iomm.codec + AC_DAC_FIFOS_REG, (1 << 6));
    break;
  default:
  ...
  case SNDRV_PCM_FORMAT_S16:
    suniv_clrbits(iomm.codec + AC_DAC_FIFOS_REG, (1 << 5));
    break;
  default:
  ...
  ret = readl(iomm.codec + AC_DAC_FIFOS_REG);
  ret&= 0x1fffffff;
  ...
  writel(ret, iomm.codec + AC_DAC_FIFOS_REG);
  ...
  //printk("%s, dma bytes %d\n", __func__, prtd->dma_period);
  writel(prtd->dma_period, iomm.dma + NDMA0_BYTE_CNT_REG);
  return 0;
}
...
{ 
  ...
  case SNDRV_PCM_TRIGGER_PAUSE_RELEASE:
    iomm.dma_trigger = 1;
    suniv_setbits(iomm.codec + AC_DAC_FIFOS_REG, 1);
		suniv_setbits(iomm.dma + NDMA0_CFG_REG, (1 << 31));
    break;
...
static int mypcm_new(struct snd_soc_pcm_runtime *rtd)
{
  ...
  writel(0x01ac8190, iomm.dma + NDMA0_CFG_REG); // 0x012c8102, 0x01220102, 0x02220202
  ...
  printk("miyoo audio dma_virt: 0x%08x, dma_phys: 0x%08x\n", (unsigned int)iomm.dma_virt, (unsigned int)iomm.dma_phys);
...
static int myaudio_probe(struct platform_device *pdev)
{
  ...
  printk("%s, irq %d\n", __func__, irq);
	if(request_irq(irq, dma_irq_handler, IRQF_SHARED, "miyoo_dma_irq", ss)) {
